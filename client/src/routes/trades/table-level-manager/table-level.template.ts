...
import type { TradesTableLevelManager } from './table-level';
...
export const TableLevelTemplate = html<TradesTableLevelManager>`
    ${whenElse(
        (x) => getViewUpdateRightComponent(x.user, 'TradeViewFull'),
        html`
            <entity-management
                ...
                :columns=${(x) => getViewUpdateRightComponent(x.user, '')? columnFullDefs : columnDefs }
